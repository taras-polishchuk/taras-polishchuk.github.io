...
# Each entry: id -> {term (display), short (one-line tooltip), long (modal body)}
# ============================================================================
...
def render_glossary_data():
    """Build a JS object literal for the glossary, for hover lookup."""
    items = []
    for gid, g in GLOSSARY.items():
        items.append(
            f"  {js_literal(gid)}: {{id:{js_literal(gid)},term:{js_literal(g['term'])},"
            f"short:{js_literal(g['short'])},long:{js_literal(g['long'])}}}"
        )
...
def page_html(lang, qa_html, lang_label, other_lang_url, other_lang_label):
    ...
    js_glossary = render_glossary_data()
...
<div class="wrap">
  ...
  <footer class="foot">
    ...
    <div style="margin-top:8px">
      Hover any <span class="term" data-term="ollama">underlined term</span> for a one-line tooltip. Click it for the full definition.
      Press <kbd>/</kbd> to <span class="term" data-term="idempotent">jump</span> to a term via the glossary panel.
    </div>
  </footer>
...
<button class="glossary-toggle" id="gp-toggle">📖 Glossary</button>
<div id="glossary-panel">
  <button class="gp-close" id="gp-close" aria-label="Close">×</button>
  <h3>Glossary · {len(GLOSSARY)} terms</h3>
  <input type="text" class="gp-search" id="gp-search" placeholder="Search terms…" autocomplete="off">
  <div id="gp-list"></div>
</div>
